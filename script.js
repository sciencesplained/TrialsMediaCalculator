...
function calculateMain() {
   ...
   // Determine the correct label based on the checkbox
    const staffLabel = includeOverhead
    ? 'Staff Cost (incl. 25% overhead)'
    : 'Staff Cost (excl. overhead)';
    ...
    document.getElementById("costBreakdown").innerHTML = `
      ...
      <ul>
        <li><strong>${staffLabel}:</strong> £${Math.round(totalStaffCost).toLocaleString()}</li>
  ...
  function calculateROI() {
    ...
    const effectiveStaffRate = staffCost * fte * overheadMultiplier;

    const overrunMonths = [0, 1, 3];
    const extendedDurations = overrunMonths.map(m => baseDuration + m);

    // No‐media costs include overhead automatically
    const noMediaCosts = extendedDurations.map(d => d * effectiveStaffRate);


    // Media‐adjusted durations & costs
    const withMediaDurations = extendedDurations.map(d => d / (1 + boost));
    const withMediaCosts = withMediaDurations.map(d => d * effectiveStaffRate + investment);
